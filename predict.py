...
def predict_prices(model, dataset, scalers, device, num_samples=5):
    cost_names = ['Seed', 'Fertilizer', 'Herbicide', 'Pesticide', 'Labor']
...
def predict_future_prices(model, dataset, scalers, device, region, district, crop, acres=1, forecast_months=6):
    ...
    regional_inflation_mult = REGIONAL_INFLATION.get(region, 1.0)
    ...
    with torch.no_grad():
        for i in range(forecast_months):
            ...
            supply_chain_factor = np.random.uniform(1 - SUPPLY_CHAIN_VOLATILITY, 1 + SUPPLY_CHAIN_VOLATILITY, size=5)
